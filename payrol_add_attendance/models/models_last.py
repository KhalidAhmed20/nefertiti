...
class EditHrPayslip(models.Model):
    ...
    def get_overtime_payrol_ids(self):
        overtime_payr = self.env['hr.overtime'].search([
        ...
        if overtime_payr:
            ...
            self.overtime_payrol_ids = False

    #Fridays
    @api.depends('employee_id', 'date_from', 'date_to', )
    def get_fridays_ids(self):
        fridays = self.env['hr.employee'].sudo().search([
        ...
        if fridays:
            for friday in fridays.fridays_ids:
                if friday.date_from >= self.date_from and friday.date_to <= self.date_to:
                    list.append(friday.id)
                else:
                    self.fridays_payroll_ids = self.fridays_payroll_ids
            self.fridays_payroll_ids = list
        else:
            self.fridays_payroll_ids = False

    # The Rest
    @api.depends('employee_id', 'date_from', 'date_to', )
    def get_rest_ids(self):
        rests = self.env['hr.employee'].search([
            ('id', '=', self.employee_id.id),
            ('the_rest_ids', '!=', False),
        ], limit=1)
        list = []
        if rests:
            for rest in rests.the_rest_ids:
                if rest.date_from >= self.date_from and rest.date_to <= self.date_to:
                    list.append(rest.id)
                else:
                    self.rest_payroll_ids = self.rest_payroll_ids
            self.rest_payroll_ids = list
        else:
            self.rest_payroll_ids = False
    ...
    def get_days_penalties(self):
        self.total_deduction_edit = self.total_deduction_edit
        for rec in self:
        ...
        self.total_penalties = days_penalties
        if self.frist_time or self.second_time or self.third_time or self.fourth_time \
                or self.fifth_time or self.total_penalties or self.absence_without_permission:
            self.total_deduction = self.frist_time + self.second_time + self.third_time + self.fourth_time + \
                                   self.fifth_time + self.total_penalties + self.absence_without_permission
            self.total_penalties_edit = self.frist_time + self.second_time + self.third_time + self.fourth_time + \
                                   self.fifth_time
            self.total_deduction_edit = self.total_penalties + self.total_penalties_edit
        else:
            self.total_deduction = False
    ...
    def get_rest_allowance(self):
        for rec in self:
            the_coefficient = self.env['hr.employee'].search([
                ('id', '=', self.employee_id.id),])
            rec.rest_discount = (rec.rest_allowance * rec.struct_id.struct_number) * (the_coefficient.the_coefficient) - (rec.rest_allowance * rec.struct_id.struct_number)
