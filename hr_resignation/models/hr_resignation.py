...
class HrResignation(models.Model):
    ...
    resignation_reason = fields.Selection(
        [('resign', 'استقالة'), ('not_valid', 'عدم صلاحية'), ('break_from_work', 'انقطاع عن العمل'), ('death', 'وفاة'),
         ('contract_finished', 'انهاء تعاقد'),('illness', 'عجز مرضي'),('fault_fired', 'فصل لخطأ جسيم'),('retired', 'تسوية معاش'),('problem', 'فصل نزاع قضائي')],
            ...
            if rec.expected_revealing_date and rec.resign_confirm_date:
                ...
                if rec.expected_revealing_date <= fields.Date.today() and rec.employee_id.active:
                    ...
                    if rec.resignation_type == 'resigned':
                        ...
                        rec.employee_id.resigned_reason = rec.reason
                        print("Reason Gooooo")
                    else:
...
class HrEmployee(models.Model):
    ...
    resigned_reason = fields.Text(string="Reason", store=True)
